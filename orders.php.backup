    ...
    <div class="fashion_section" style="display: flex; justify-content: center; align-items: center;">
        ...
        <div style="display: flex; align-items: center; justify-content: center; columns: 100px 4;">
            ...
            if (isset($_COOKIE['user_id'])):
                ...
                $status_query = "SELECT status, COUNT(status) FROM purchases WHERE user_id=$user_id GROUP BY status";
                $status_search = mysqli_query($lazada, $status_query);
                $status_list = mysqli_fetch_all($status_search);
                foreach ($status_list as $key => $status):
                    $status_name = $status[0];
                    $total_status = $status[1];
                    ?>
                    <a href="?status=<?php echo $status_name ?>">
                        <p style="font-size: large; font-weight: 700;">
                            <?php echo "${status_name}(${total_status})" ?>
                        </p>
                    </a>
                <?php endforeach; ?>
            <?php endif; ?>
